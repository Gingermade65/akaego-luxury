    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
        ...
        <form action="{{ route('checkout.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-luxury-charcoal/40 border border-luxury-gold/20 p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Full Name *</label>
                            ...
                            @error('customer_name') <span class="text-xs text-red-400">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Email Address *</label>
                            <input type="email" name="customer_email" value="{{ old('customer_email', auth()->user()->email ?? '') }}" required
                                   class="w-full bg-luxury-black border border-luxury-gold/30 px-4 py-2 text-sm text-luxury-cream focus:outline-none focus:border-luxury-gold">
                            @error('customer_email') <span class="text-xs text-red-400">{{ $message }}</span> @enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Phone Number</label>
                ...
                <div class="bg-luxury-charcoal/40 border border-luxury-gold/20 p-6 space-y-4">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Street Address *</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">City *</label>
                                <input type="text" name="shipping_city" value="{{ old('shipping_city') }}" required
                                       class="w-full bg-luxury-black border border-luxury-gold/30 px-4 py-2 text-sm text-luxury-cream focus:outline-none focus:border-luxury-gold">
                            </div>

                            <div>
                                <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">State / Region</label>
                                <input type="text" name="shipping_state" value="{{ old('shipping_state') }}"
                                       class="w-full bg-luxury-black border border-luxury-gold/30 px-4 py-2 text-sm text-luxury-cream focus:outline-none focus:border-luxury-gold">
                            </div>

                            <div>
                                <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Postal Code *</label>
                        ...
                        <div>
                            <label class="block text-xs uppercase tracking-wider text-luxury-cream/70 mb-1">Country *</label>
                ...
                <div class="bg-luxury-charcoal/40 border border-luxury-gold/20 p-6 space-y-4">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center space-x-3 p-3 bg-luxury-black border border-luxury-gold/20 cursor-pointer">
                            <input type="radio" name="payment_method" value="stripe" checked class="text-luxury-gold focus:ring-0">
                            <span class="text-sm">Credit / Debit Card (Stripe)</span>
                        </label>

                        <label class="flex items-center space-x-3 p-3 bg-luxury-black border border-luxury-gold/20 cursor-pointer">
                            <input type="radio" name="payment_method" value="cod" class="text-luxury-gold focus:ring-0">
                            <span class="text-sm">Private Concierge Payment on Delivery</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-luxury-charcoal/60 border border-luxury-gold/30 p-6 h-fit space-y-6">
                <h2 class="font-serif text-xl text-luxury-gold tracking-widest uppercase border-b border-luxury-gold/20 pb-4">
                    Order Summary
                </h2>
                ...
                <div class="space-y-4 max-h-64 overflow-y-auto pr-2">
                    @foreach($cart as $id => $item)
                        <div class="flex justify-between text-xs">
                            ...
                            <span class="text-luxury-gold font-mono">${{ number_format($item['price'] * $item['quantity'], 2) }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="border-t border-luxury-gold/20 pt-4 space-y-2 text-sm">
                    ...
                    <div class="flex justify-between text-luxury-cream/70">
                        ...
                        <span class="font-mono">
                            {{ $shippingFee === 0.00 ? 'COMPLIMENTARY' : '$' . number_format($shippingFee, 2) }}
                        </span>
                    </div>
                    <div class="border-t border-luxury-gold/20 pt-3 flex justify-between font-serif text-lg text-luxury-gold">
                        <span>Total</span>
